#!/usr/bin/env node
// Converte ROTEIROS_CAMINHOS.md → ROTEIROS_CAMINHOS.pdf usando Chrome headless.
...
const MD = path.join(ROOT, 'ROTEIROS_CAMINHOS.md');
const HTML_TMP = path.join(ROOT, '.tmp-caminhos.html');
const PDF = path.join(ROOT, 'ROTEIROS_CAMINHOS.pdf');
